...
/**
*内核中实现了socket_alloc结构，将socket和vfs_inode封装在一起，
*这样通过vfs_inode和socket都可以互相找到对方，同时又保证了inode的独立性，
*适用于各种文件类型。
*fsocket_alloc封装了socket_alloc
*/
...
/**
*在抢占式内核中，有三处地方需要显示的禁用抢占：
*		操作Per-CPU变量的时候
*由于内核是抢占性的，内核中的进程在任何时刻都可能停下来以便另一个具有更高优先级的进程运行。
*这意味着一个任务与被抢占的任务可能会在同一个临界区内运行。
*为了避免这样的情况，内核抢占代码使用自旋锁作为非抢占区域的标记。如果一个自旋锁被持有，
*内核便不能进行抢占。因为内核抢占和SMP面对相同的并发问题，并且内核已经是SMP安全的，
*因此，这种简单的变化使得内核也是抢占安全的。
*实际中，某些情况并不需要自旋锁，但是仍然需要关闭内核抢占。出现得最频繁的情况就是每个
*处理器上的数据。如果数据对每个处理器是唯一的，那么这样的数据可能就不需要使用锁来保护，
*因为数据只能被一个处理器访问。如果自旋锁没有被持有，内核有是抢占式的，那么一个新调度的
*任务就可能访问同一个变量。为了解决这个问题，可以通过preempt_disable()禁止内核抢占。
*这是一个可以嵌套调用的函数，可以调用任意次。每次调用都必须有一个相应的preempt_enable()调用。
*当最后一次preempt_enable()被调用后，内核抢占才重新启用。	
*/
		preempt_disable();
		if (fsock_pool->free_cnt) {
			//取得这个链表第一元素的所指结构体的首地址
...
/**
*迁移Socket的情况
*不是很懂为什么src的free_cnt减少,dst的free_cnt增加
*/
...
/**
*分配dentry,貌似只用了一些必要的,其余为了兼容全设置成了NULL
*/
...
struct socket_alloc结构体中包含了struct socket 和struct inode(struct inode结构体，
是linux内核用来刻画一个存放在内存中的文件的,通过将struct inode 和 struct socket绑定在一起形成
struct socket_alloc结构体,来表示内核中的网络文件)
*/
...
/**
*start:主要实现初始化工作，在遍历一个链接对象开始时，调用。
*返回一个链接对象的偏移或者SEQ_START_TOKEN（表征这是所有循环的开始）。出错返回ERR_PTR。
*stop():当所有链接对象遍历结束时调用。主要完成一些清理工作。
*next():用来在遍历中寻找下一个链接对象。返回下一个链接对象或者NULL（遍历结束）。
*show():对遍历对象进行操作的函数。主要是调用seq_printf(), seq_puts()之类的函数，
*打印出这个对象节点的信息。
*/
...
int fsocket_init(void)
{
	int ret;
/**
*专用高速缓存的创建函数kmem_cache_create()
*内核函数 kmem_cache_create 用来创建一个新缓存。
...
/**
*大多数网络功能会在引导时或在模块加载时其初始化期间在/proc中注册一个或多个文件。
*当用户读取该文件时，会引起内核间接运行一组内核函数，返回某种输出内容。
*网络代码所注册的文件位于/proc/net。
* /proc中的目录可以使用proc_mkdir创建。/proc/net中的文件可以使用定义在
*include/linux/proc_fs.h中的proc_net_fops_create和proc_net_remove予以注册和注销。
*还有两个通用的API函数create_proc_entry和remove_proc_entry。
*proc_net_fops_create负责调用proc_net_create创建文档，然后初始化器文件操作处理函数。
*/	
