...
/*枚举fastsocket的所有状态，并且最后一个枚举量表示状态总数*/
enum {
...
/*下面是对于fsocket状态进行增减的宏函数，因为对于每个cpu来说，这个fsocket状态表示它有多少个socket处于哪种状态*/
/*****************************************************************************************************/
...
static struct socket_alloc *fsocket_alloc_socket_mem(void)
{
	...
	if (enable_socket_pool_size) {
		...
		if (likely(fsock_alloc)) {//如果从池中取出了一块内存，则调用goto，这里goto指向一个通过这个内存分配socket的函数
			goto alloc_from_pool;
		}

		if (fsock_pool->backlog_cnt) {			//此时由于free-cnt不够，只能使用backlog-cnt
...
{
	struct fsocket_alloc *pos, *next;						//将fsocket移动到cpu_id
	int i = 0;

	list_for_each_entry_safe(pos, next, &src_pool->free_list, next) {		//删除本地id的fsocket结构
		list_del(&pos->next);
		src_pool->free_cnt--;
		FSOCKET_DEC_STATS(FSOCKET_STATS_SOCK_IN_POOL);
		...
		// Change CPU ID
		pos->cpu_id = cpu_id;
		list_add(&pos->next, &dst_pool->free_list);				//加入cpu_id的池？
		dst_pool->free_cnt++;
...
static int fsock_alloc_file(struct socket *sock, struct file **f, int flags)
{
	...
	path.mnt = fastsocket_mnt;//fastsocket global variable
	/*获取文件操作*/
...
static void fsocket_init_socket(struct socket *sock)
{
	if (enable_direct_tcp) {
		sock_set_flag(sock->sk, SOCK_DIRECT_TCP);//socket 优化参数？
...
/*read here first time 4/17/2017 adalos*/
...
int fsocket_socket(int flags)
{
	...
	fd = fsock_map_fd(sock, flags);//利用这个函数来产生sock对fd的映射，可以减少锁
	if (fd < 0) {
		err = fd;
...
int fsocket_spawn(struct file *filp, int fd, int tcpu)
{
	...
	ret = fsocket_process_affinity_check(tcpu);//检查函数的亲和性
	if (ret < 0) {
	...
	cpu = ret;
	newsock = NULL;
	ret = fsocket_spawn_clone(fd, sock, &newsock);//将原来的sock拷到newsock里面
	if (ret < 0) {
